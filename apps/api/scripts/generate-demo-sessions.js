...
function buildPlanForDay(day, dayOffset, isToday) {
    const slots = [
        { hour: 8, minute: 20 },
        { hour: 10, minute: 35 },
        { hour: 14, minute: 20 },
...
async function main() {
    ...
    if (dryRun) {
        ...
        rows.forEach((row) => {
            console.log(`${row.startTime.toLocaleString()} | ${row.durationMinutes}m (+${row.breakMinutes}m break) | course=${row.courseId}`);
        });
...
//# sourceMappingURL=generate-demo-sessions.js.map